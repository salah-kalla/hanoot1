...
import 'package:Hanoot/states/homescreen/kitchen_state.dart';
...
class Kitchen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<KitchenState>(builder: (context, state, child) {
      return Column(
        children: <Widget>[
          Padding(
            padding:
            const EdgeInsets.only(right: 10, left: 40, top: 10, bottom: 2),
            child: Container(
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: <Widget>[
                  Text("المطبخ", style: GoogleFonts.elMessiri(fontSize: 20)),
                  InkWell(
                      onTap: () {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (_) => ProductListCategory(
                                  categoryId: 214,
                                  isSeen: true,
                                ),
